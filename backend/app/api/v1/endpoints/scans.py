...
from app.db.models import Scan, ScanFrame, SkinProfile, ScanStatus, Product
...
from app.services.skin_analysis import analyze_skin_profile, AnalysisUnavailableError

router = APIRouter()


def _profile_response(profile: SkinProfile) -> SkinProfileResponse:
...
def analyze_scan(scan_id: uuid.UUID, db: Session = Depends(get_db)):
    ...
    if not frame_paths:
        raise HTTPException(status_code=400, detail="No stored frame images found for this scan")

    # Analyzing the same scan twice must not create duplicate profiles/snapshots.
    existing_profile = db.query(SkinProfile).filter(SkinProfile.scan_id == scan_id).first()
    if existing_profile:
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Skin analysis engine is unavailable: {exc}")
    ...
    db.add(profile)
    db.flush()  # populates profile.created_at, needed for day_offset below
